...
dados_disponibilidade = {
    'servico': ['ERP', 'Portal do Cliente', 'Sistema de Pagamentos', 'E-mail Corporativo'],
...
cores = ['red' if d < meta_disponibilidade else 'steelblue' for d in df_disp['disponibilidade_pct']]

plt.figure(figsize=(9, 4))
plt.bar(df_disp['servico'], df_disp['disponibilidade_pct'], color=cores)
plt.axhline(y=meta_disponibilidade, color='orange', linestyle='--', label=f'Meta SLA ({meta_disponibilidade}%)')
